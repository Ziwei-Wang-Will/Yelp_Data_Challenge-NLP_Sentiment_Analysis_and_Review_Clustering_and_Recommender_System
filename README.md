...
3. Sentiment Classification Results Analysis and Insights 
4. Review Clustering
5. Other User Cases of Clustering
6. Recommender System Model Fitting and Models Comparison
7. Recommendation Results Analysis and Insights    
        ...
        - Did standardized and standardized&PCA preprocessing improve the basic model performce by comparing the AUCs and Accuracies? 
            - Only performed **standardization without PCA**
                ...
                - Had nagetive impact on **KNN**.
                    - Perhaps when we reduced the value scale of all dimensions, we reduced the value scale of distance between data points, thus, reduced the distinguish power of our model.
            - Perform **standardization with PCA** 
                - No significant impact on performances of **Logistic Regression**.
                    - Perhaps the previous basic **Logistic Regression** model had already performed normalization which essentially reduced feature dimensions, thus the PCA played the same role and had no impact on the model performance.
                - Improved the performance of **KNN**.
                    - Before we performed PCA, our basic model might suffer from **Curse of Dimensionality** which means nearest neighbors can be 'far' in high dimension. 
                    - **KNN**, or any method involving this sort of distancing, suffers majorly from **curse of dimensionality**.
                    - PCA reduced feature dimensions and negative effect of **curse of dimensionality** to improve the performnce of **KNN**, especially the accuracy.
                - Had a negative impact on the overfitting issue of **Random Forest** that provided a more unstable result.
                    - Different between trian_acu and test_auc became larger, which indicated overfitting and unstability.
...
- - [**Detailed Code and Plotting**](https://github.com/will-zw-wang/Yelp_Data_Challenge-NLP_Sentiment_Analysis_and_Review_Clustering_and_Recommender_System/blob/master/code/Yelp_Data_Challenge%20-%20NLP.ipynb)   

### 3. Sentiment Classification Results Analysis and Insights
- What were the key features (words) that made the positive prediction?(Generated by **Logistic Regression** Classifier)
    - <img src="https://github.com/will-zw-wang/Yelp_Data_Challenge-NLP_Sentiment_Analysis_and_Review_Clustering_and_Recommender_System/blob/master/images/sentiment%20classification/top_words_positive_review_by_LR.png" width="140" height="400">
    - The top 20 key features(words) that made the positive prediction including:'amazing','phenomenal', 'delicious', etc. 
    - It's reasonable to use these positive words to make the positive prediction (stars == 5).
    - Users paid large attention on environment, taste of food, ect.
- What were the key features (words) that made the negative prediction?(Generated by **Logistic Regression** Classifier)
    - <img src="https://github.com/will-zw-wang/Yelp_Data_Challenge-NLP_Sentiment_Analysis_and_Review_Clustering_and_Recommender_System/blob/master/images/sentiment%20classification/top_words_negative_review_by_LR.png" width="140" height="400">
    - The top 20 key features(words) that made the negative prediction including: 'bland', 'rude', 'unprofessional', 'slow', 'overpriced', 'unfortunately', ect.
    - Users complained lots about unsatisfactory service, overpriced food, tasteless food, ect.
    - Moreover, one interesting point was that neutral words such as 'ok' and 'okay' were appeared in the negative reviews.
        - Probably, some customers left these neutral words but still rated a restaurant less than 5 stars. 
